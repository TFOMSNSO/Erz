	...
	protected void createMiddle(int count, Namespace namespace,	Element rootElement, String curDate,boolean tt,String kluch)
	{
			...
			if(kluch.equals("otherzp9")){
				for (int i = 0; i < count; i++)
				{
					...
					if(dataList.get(i).get(ENP) != null && dataList.get(i).get(ENP).length() > 0){
						Element qpd5_0 = new Element("QPD.5", namespace);
						qpd.addContent(qpd5_0);
						qpd5_0.addContent(new Element("CX.1",namespace).addContent(dataList.get(i).get(ENP)));
						qpd5_0.addContent(new Element("CX.5", namespace).addContent("NI"));
					}

					if(dataList.get(i).get(PERSON_SERDOC) != null &&
						dataList.get(i).get(PERSON_NUMDOC) != null && (dataList.get(i).get(PERSON_SERDOC).length() + dataList.get(i).get(PERSON_NUMDOC).length()) > 0) {
						Element qpd5_1 = new Element("QPD.5", namespace);
						qpd.addContent(qpd5_1);
						qpd5_1.addContent(new Element("CX.1", namespace)
								.addContent(
						...
						qpd5_1.addContent(new Element("CX.5", namespace).addContent("14"));
					}


					if(dataList.get(i).get(SNILS)!= null && dataList.get(i).get(SNILS).length() > 0) {
