...
public class MessageA03p07other extends MessageCommon {

    public MessageA03p07other() {
        super();
        // TODO Auto-generated constructor stub
    }
    public MessageA03p07other(int pERSON_SERDOC, int pERSON_NUMDOC,
    ...
    //a03p07 для людей с другой территории
    @Override
    ...
    protected void createMiddle(int count, Namespace namespace, Element rootElement, String curDate, boolean tt) {
        for (int i = 0; i < count; i++) {
            ...
            if(dataList.get(i).get(PERSON_DOCPERSONID).equals("")) {
                pid3_1.addContent(new Element("CX.5", namespace).addContent("14"));
            }else{
            ...
            if(!dataList.get(i).get(SNILS).equals("")){
                Element pid3_4 = new Element("PID.3", namespace);
                pid.addContent(pid3_4);
                pid3_4.addContent(new Element("CX.1",namespace).addContent(dataList.get(i).get(SNILS)));
                pid3_4.addContent(new Element("CX.5",namespace).addContent("PEN"));
            ...
            if(dataList.get(i).get(PERSON_KINDLASTNAME).equals("-") || dataList.get(i).get(PERSON_KINDLASTNAME).equals("НЕТ")){
