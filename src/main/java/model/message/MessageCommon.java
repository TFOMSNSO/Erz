...
public abstract class MessageCommon implements Message, MessagePrizyvOnly{
	...
	public boolean create(String userMachine) {
		...
		//для а08п06 проставляем в person_enp_output pack_id = 100
	...
	@Override
	public boolean createPrizyv(String userMachine){
		long before = System.currentTimeMillis();
...
//				System.out.println("rememberGuid" + i);
	...
	void listGuidFiod(Object guidMsh, int i, String mes_type){
		personEnpGuidRow = new ArrayList<String>();
		personEnpGuidRow.add(dataList.get(i).get(PERSON_SERDOC) + dataList.get(i).get(PERSON_NUMDOC));
	...
	*/			pid3_5.addContent(new Element("CX.7", namespace).addContent(d1));
		...
		if(! dataList.get(i).get(RUSSIAN).equals("RUS")){
			if(dataList.get(i).get(KATEG).equals("5") || dataList.get(i).get(KATEG).equals("10")){
				...
				pid26_2.addContent(new Element("CWE.3", namespace).addContent("1.2.643.2.40.3.3.0.6.19"));
			}else if(dataList.get(i).get(KATEG).equals("9") || dataList.get(i).get(KATEG).equals("4")){
				Element pid26_2 = new Element("PID.26", namespace);
				pid.addContent(pid26_2);
				pid26_2.addContent(new Element("CWE.1", namespace).addContent("4"));
				pid26_2.addContent(new Element("CWE.3", namespace).addContent("1.2.643.2.40.3.3.0.6.19"));
	...
	public boolean create(String userMachine,ArrayList<ArrayList<String>> listList1,String kluch) {
		...
		bhs.addContent(new Element("BHS.7", namespace).addContent(curDate));
		guidBhs = new RandomGUID();
//		System.out.println("guid:" + guidBhs);
		bhs.addContent(new Element("BHS.11", namespace).addContent(guidBhs.toString()));
		...
		if(kluch.equals("otherzp9")){
			createMiddle(count,namespace,rootElement,curDate,true,kluch);
		}
		else if(kluch.equals("list1enpzp9"))
		{
		...
		else
		{
			...
			else
			{
				if(kluch.equals("list1snilszp9"))
				{
					...
					else if(kluch.equals("zp1fiodtable")){
						createMiddle(count,namespace,rootElement,curDate,true,kluch);
					}else{
...
//				System.out.println(prepareData + ")" + " size = " + dataListRow.size() + " :::" + dataListRow);
	...
	public MessageCommon(int PERSON_SERDOC, int PERSON_NUMDOC, int PERSON_DOCPERSONID, int PERSON_SURNAME, int PERSON_KINDFIRSTNAME, int PERSON_KINDLASTNAME, int PERSON_BIRTHDAY,
						 int PERSON_SEX, int PERSON_LINKSMOESTABLISHMENTID, int ENP, int PERSON_ADDRESSID, int PERSON_DATEINPUT, int SNILS, int BORN, int DATEPASSPORT, int ENP_PA,
						 int VS_NUM, int VS_DATE, int ZAD, int d2, int SMO, int d_12, int d_13, int OKATO_3, int TYPE_POL, int POL, int ENP_1, int ENP_2, int p14CX1, int p14CX5,
						 int p14CX6, int p14CX7, int XPN1, int XPN2, int XPN3, int USERNAME, int ZADMINUS1, int ZADPLUS40, int NBLANC, int VS_DATEPLUS1, int USER_ENP, int USER_PERSON_SURNAME,
						 int USER_PERSON_KINDFIRSTNAME, int USER_PERSON_KINDLASTNAME, int USER_SMO, int USER_D_12, int USER_D_13, int USER_OKATO_3, int USER_TYPE_POL, int USER_POL, int RUSSIAN,
						 int d_V, int d_SER, int d_NUM, int PR_FAM, int PR_IM, int PR_OT, int LAST_FAM, int LAST_IM, int LAST_OT, int LAST_DR, int PFR_SNILS, int PFR_ID, int PFR_NOTID,
						 int USER_SERDOC, int USER_NUMDOC, int USER_DOCID, int USER_DOC_DATE, int d_12_PLUS1, int KATEG, int pid29, int d1) {
		...
		D_12 = d_12;
		D_13 = d_13;
		this.OKATO_3 = OKATO_3;
		this.TYPE_POL = TYPE_POL;
		this.POL = POL;
		this.ENP_1 = ENP_1;
		this.ENP_2 = ENP_2;
		P14CX1 = p14CX1;
		...
		this.ZADMINUS1 = ZADMINUS1;
		this.ZADPLUS40 = ZADPLUS40;
		this.NBLANC = NBLANC;
		this.VS_DATEPLUS1 = VS_DATEPLUS1;
		...
		this.PFR_SNILS = PFR_SNILS;
		this.PFR_ID = PFR_ID;
		this.PFR_NOTID = PFR_NOTID;
		...
		while (resultSet.next())
		{
			...
			for (int i = 0; i < numcols; i++) {
				if (resultSet.getObject(i + 1) != null && !"null".equals(resultSet.getObject(i + 1)))
				{
						...
						// пребразуем даты
						if (i == 3 || i == 13 || i == 24 || i == 25 || i == 32 || i == 41 || i == 57 || i == 60 || i==59 || i == 58)
						{
...
/*	 System.out.println("!!!!!!!!!!!!!!!!!!!!!!!!!!! "+list.get(1).size());
 ...
 private String parseStringDateYYY_MM_DD(String cunvertCurrentDate) throws ParseException
 {
	...
	@Override
	public void setPERSON_SERDOC(int i) {
 		this.PERSON_SERDOC = i;
	}

	@Override
	public void setPERSON_NUMDOC(int i) {
		this.PERSON_NUMDOC = i;
	}

	@Override
	public void setSNILS(int i) {
		this.SNILS = i;
	}

	@Override
	public void setEnp(int i) {
		this.ENP = i;
	}
}
