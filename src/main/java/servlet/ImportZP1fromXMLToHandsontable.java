...
//	  System.out.println("parsed uprak2:" + parsedUprak2);
	...
	if(kluch != null && (kluch.equals("zp1fiodtable") || kluch.equals("A03P07other") || kluch.equals("otherzp9"))){
		compare2(parsedatauprmessZP1,parsedatauprak2ZP1,kluch);
	}else{
 ...
 protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
  ...
  private ArrayList<ArrayList<String>>  parse(ArrayList<ArrayList<String>> list)
  {
	...
	private void compare2(ArrayList<ArrayList<String>> uprmes,ArrayList<ArrayList<String>> uprak, String kluch){
		try {
			if(kluch.equals("otherzp9")){
				for (int i = 0; i < uprmes.size(); i++) {
					for (int j = 0; j < uprak.size(); j++) {
						if(uprmes.get(i).get(4).equals(uprak.get(j).get(0).trim())){
							uprak.get(j).add(0, uprmes.get(i).get(3));
							uprak.get(j).add(0, uprmes.get(i).get(2));
							uprak.get(j).add(0, uprmes.get(i).get(1));
							uprak.get(j).add(0, "-");
							uprak.get(j).add(0, uprmes.get(i).get(0));
						}
					}
				}
			}else{
				for (int i = 0; i < uprmes.size(); i++) {
					for (int j = 0; j < uprak.size(); j++) {
						if (uprmes.get(i).get(8).trim().equals(uprak.get(j).get(0).trim())) {
							uprak.get(j).add(0, uprmes.get(i).get(3));
							uprak.get(j).add(0, uprmes.get(i).get(2));
							uprak.get(j).add(0, uprmes.get(i).get(1));
							uprak.get(j).add(0, uprmes.get(i).get(0));
			...
			System.err.println("importzp1fromxml error:" + e.getMessage());
		}
	}
...
//					
						...
						parsedatauprmessZP1.get(i).add("noUdl");
					}
					...
					if (parsedatauprak2ZP1.get(j).get(6).trim().equals(parsedatauprmessZP1.get(i).get(39).trim())) {
						...
						parsedatauprmessZP1.get(i).add("noEnp");
					}
