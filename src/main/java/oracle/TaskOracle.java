...
public class TaskOracle extends ConnectOracle {
	...
	//удачи разобраться в этом прекрасном проекте
	public ResultSet selectData(Statement statement, String username) throws SQLException {
		...
		ResultSet resultSet = statement.executeQuery("select p.person_serdoc, p.person_numdoc,  \n" +
				...
				"  nvl(     (case  when (pa.snils is not null) and length(replace(replace(pa.snils,'-'),' ')) = 11 and \n" +
				"    pfr.kod_ss@pfr(substr(replace(replace(pa.snils,'-'),' '),1,9)) =  substr(replace(replace(pa.snils,'-'),' '),10,2)  \n" +
				"    then replace(replace(pa.snils,'-'),' ') else null end),     \n" +
				"      replace(replace(pfr.fiod_ss@pfr(p.person_surname, p.person_kindfirstname, p.person_kindlastname, p.person_birthday),'-'),' ')) snils  ,\n" +
				"       pa.born, trunc(pa.datepassport) datepassport, pa.enp enp_pa, pa.vs_num, pa.vs_date,  pa.zad, pa.d2,  pi.smo, pi.d_12, pi.d_13, \n" +
				"       pi.okato_3, pi.type_pol, pi.pol, pi.enp_1, pi.enp_2,  z.p14cx1, z.p14cx5, z.p14cx6, z.p14cx7, z.xpn1, z.xpn2, z.xpn3,  xt.username, \n" +
				"        pa.zad - 1 zadminus1, pa.zad + 40 zadplus40,  ERZ.n_blank (pa.enp, pa.vs_date) nblanc,  pa.vs_date + 1 vs_dateplus1,  xt.user_enp, \n" +
				"        xt.user_person_surname, xt.user_person_kindfirstname, xt.user_person_kindlastname, xt.user_smo,  trunc(xt.user_d_12) user_d_12, \n" +
				"        trunc(xt.user_d_13) user_d_13, xt.user_okato_3, xt.user_type_pol, xt.user_pol,  nvl((select alfa3 from developer.oksm o where o.kod = \n" +
				"        russian and rownum = 1),'RUS'), pa.d_v, pa.d_ser, pa.d_num, pa.pr_fam, pa.pr_im, pa.pr_ot, pa.last_fam, pa.last_im, pa.last_ot,\n" +
				"         pa.last_dr,  xt.pfr_snils, xt.pfr_id, xt.pfr_notid, xt.USER_SERDOC, xt.USER_NUMDOC, xt.USER_DOCID, xt.USER_DOC_DATE,  trunc(pi.d_12 + 1) D_12_PLUS1, \n" +
				"         pa.kateg, \n" +
				"         nvl((select  max(to_char(to_date(d_end,'YYYYMMDD'),'YYYY-MM-DD')) from goznak_csv  where enp = pa.enp and d_insert = (select max(d_insert) from goznak_csv where enp = pa.enp )),' ') ddddd, trunc(pa.d1)  \n" +
				"         from person p  left join personadd pa on p.person_addressid = pa.personadd_addressid  \n" +
				"         left join xml_person_enp xp on xp.enp = p.enp and rownum = 1 \n" +
				"         left join xml_person_enp_input pi \n" +
				"         on pi.gd = xp.guid  and pi.npp = 0 \n" +
				"         and rownum = 1  \n" +
				"         \n" +
				"         left join zu1 z on z.person_id = p.person_addressid  \n" +
				"         inner join xml_task xt on xt.enp = p.enp  \n" +
				"         \n" +
				"         where  xt.username = '" + username + "'  \n" +
				"order by p.enp\n");
	...
	public void insertToPersonEnpOutput(Statement statement, String enp, String guid, String mes_type, String enp_pa) {
		System.out.println("enp:" + enp + ",guid:" + guid + ",mes_type:" + mes_type + ",enp_pa:" + enp_pa);
		try {
			if(mes_type.equals("6")){//для a08p06
				statement.executeQuery("update person_enp_output set pack_id = 100 where enp = '"  + enp + "' and status = 6");
			}else{
