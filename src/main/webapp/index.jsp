...
$(document).ready(function()
{
	...
	var opts = {
	        ...
	        var gouser = "user="+ '<%=user%>';
	    	   ...
	    	   var dataNew = [['фамилия','имя','отчество','др','пол','серия док','номер док','тип док','снилс','дата смерти','OKATO','TYPE_POL','POL','	 	','	 ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ']];
						 ...
						 */	
	    	             ...
	    	             КНОПКА П02
			        	     ...
			        	     $('#A08P08').click(function ()
									...
									$('#A08P08pr').click(function ()
									{
										...
										if ($('.panel#tableexcel').is(':visible'))
										{
											...
											console.log('e1:' + e1 + ', wd1:' + wd1);
											console.log('e2:' + e2 + ', wd2:' + wd2);
											console.log('e3:' + e3 + ', wd3:' + wd3);
									...
									$('#A03P07other').click(function ()
									{
										...
										if ($('.panel#tableexcel').is(':visible'))
										{
											...
											searchInDirectory('A03P07other');
											searchInDirectory(JSON.stringify(myData));
										}
									});
	        	             ...
	        	             */  
	        	             	var idZP9;
	        	             	var datatargetmZP9;
		        	             $('#ZP9enp,#ZP9passport,#buttonZP9,#ZP9snils,#otherZP9passport,#otherZP9snils').click(function ()
				        	     {
		        	            		...
		        	            		// ловим id нажатой кнопки
	        	            	 		...
	        	            	 		if ($('.panel#tableexcel').is(':visible')) 
		        	            	    { 		 
			     	        	            	if( 'ZP9enp' === ids || 'ZP9passport' === ids || 'ZP9snils' === ids || 'otherZP9passport' === ids || 'otherZP9snils' === ids)
			     		        	            {		
		     			        	 					// берем значение с нажатой кнопки  с меню запросы ффомс (кнопка верхнего уровня)
				        	            	 			var val = $('#'+id).attr("value");
				        	            	 			// обновляем данные из таблицы
					        	            	 		...
					        	            	 		// вычисляем количество строк
			      		        				          ...
			      		        				          if(ids == 'otherZP9passport' || ids == 'otherZP9snils'){
			      		        				              var myData = {
			      		        				                  list1otherzp9:inst1Lista.getData(1,0,e1-1,3)
															  }
														  }
							...
							*/
								...
								*/
									 ...
									 */
		        	            		 ...
		        	            		 if ($('.panel#tableexcel').is(':visible'))
		        	     				 {
		        	            		 		...
		        	            		 		var myData ={list1:hotInstance.getData()  }
			        	        	        	   ...
			        	        	        	   var row1 = myData.list1[0];
                                             	console.log("P06 data:" + JSON.stringify(row1));
						        	        	   $.ajax({
				        						        url: "TaskP06",
				        						        type: 'POST',
				        						        dataType: 'json',
				        						        ...
				        						        contentType: 'application/json',
				        						        success: function (data) {
				        						            // console.log(hotInstance.getData(0,30));
		        	            	 ...
		        	            	 $('#taskGzNoPolice').click(function () {
                                         $('#myModal322').modal('hide');
                                         if ($('.panel#tableexcel').is(':visible')) {
                                             var hotInstance = $('#list1onsc').handsontable('getInstance');
                                             var form = document.getElementById("formGz");
                                             var dateBegin = form.date_begin.value;
                                             var dateEnd = form.date_end.value;
		        	            	 ...
		        	            	 $('#taskGenEnpButton').click(function () {
                                         var c = confirm("Генерируем енп?");

										 if (c) {
                                             $('#myModal323').modal('hide');
                                             if ($('.panel#tableexcel').is(':visible')) {
                                                 var hotInstance = $('#list1onsc').handsontable('getInstance');
                                                 var e1 =hotInstance.countRows()-hotInstance.countEmptyRows(true);
                                                 var vids = hotInstance.getData(1,0,e1-1,0);

                                                 $("#dim3").css("height", $(document).height());
                                                 $("#dim3").fadeIn();
                                                 spinner.spin($('#spinner_center2')[0]);
                                                 ajax_cnt++;
												 console.log('myData:' + JSON.stringify(vids));
                                                 $.ajax({
                                                     url: "TaskGenEnp",
                                                     type: "POST",
...
$('#ZP1FIODtable').click(function ()
	     {
				...
				$('#tamessage').val("> Послан запрос "+this.value+"...");
				// отправляем ключ что это такой-то запрос
				searchInDirectory('ZP1FIODtable');
				// объект 1-го листа
			...
			<div class="collapse navbar-collapse"
				...
				<ul class="nav navbar-nav">
						...
						<div class="modal-dialog modal-lg">
							<div class="modal-content">
								...
								<div class="row">
										...
										<div class="modal fade" id="modalZP9" role="dialog"
											...
											<div class="modal-dialog">
												<div class="modal-content">
													...
													<div class="modal-body">
														<p>Для ЗЛ с другой территории:</p>
														<button type="button" id="otherZP9passport" class="btn btn-primary" value="0">Енп +- Паспорт +- Снилс</button>
													</div>
												</div>
											</div>
										</div>
									...
									<div class="col-md-4">
										<h4 class="text-center">Сообщение</h4>
										<li>
											<button class="btn btn-primary" id="menuindex"
										...
										<li>
											<button class="btn btn-primary" id="menuindex"
													...
													value="A08P03pr">А08П03pr</button>
										</li>
										<li>
											<button class="btn btn-primary" id="menuindex"
													title="Проставить дату окончания иностранцу"
													value="A08P03For">А08П03For</button>
										</li>
										<br>
										<li>
											<button class="btn btn-primary" id="menuindex"
										...
										<li>
											<button class="btn btn-primary" id="menuindex"
													title="Исправление данных, например ЕНП"
													value="A08P16">А08П16</button>
										</li>
										<li>
											<button class="btn btn-primary" id="menuindex"
													title="Смена данных УДЛ без смены страховки"
													value="A08P04">А08П04</button>
										</li>
										<li>
											<button class="btn btn-primary" id="menuindex"
													title="Объединение дубликатов общее"
													value="A24P10com">А24П10com</button>
										</li>
										<li>
											<button style="background-color: #4193ab;" type="button" id="A08P08"
													title="Открытие страховки" class="btn btn-primary"
													value="A08P08">А08П08</button>
										</li>
										<li>
											<button id="A08P08pr" style="background-color: #4193ab;"
													class="btn btn-primary"
													title="Восстановление призывников">А08П08prizyv</button>
										</li>
									</div>
									<div class="col-md-4">
										<h4 class="text-center">Открепление</h4>
										<%--<li>
										...
										<li>
											<button class="btn btn-primary" id="menuindex"
													title="Открепление призывников"
													value="A08P02priz">A08P02priz</button>
										</li>
										<li>
											<button style="background-color: #4193ab;" type="button" id="A03P07"
													class="btn btn-primary" value="A03P07"
													title="Сообщение о смерти">А03П07</button>
										</li>
										<li>
											<button id="A03P07other" style="background-color: #4193ab;"
													class="btn btn-primary"
													value="A03P07other"
													title="Сообщение о смерти ЗЛ с другой территории">А03П07other</button>
										</li>
										<li>
											<button class="btn btn-primary" id="menuindex"
													title="Отмена сообщения о смерти" value="A13P09">А13П09</button>
										</li>
										<h4 class="text-center">Классика жанра</h4>
										<li>
						...
						<ul class="dropdown-menu" id="dr2" role="menu">
							<li>
								<button class="btn btn-primary" id="drugiezaprosi"
									data-toggle="modal" data-target="#myModal">Запрос по ФИОД</button>
								...
								<button class="btn btn-primary" id="drugiezaprosi"
									...
									<button class="btn btn-primary" id="taskP06">Задание П06</button>
									<button class="btn btn-primary" id="taskGz" data-toggle="modal" data-target="#myModal322">Гознак/нет полиса</button>
									<button class="btn btn-primary" id="taskGenEnp" data-toggle="modal" data-target="#myModal323">Сгенерировать енп</button>
					...
					<div class="modal fade" id="importFromExcel" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
						<div class="modal-dialog">
							<div class="modal-content">
								<div class="modabtnstarttaskl-header">
				...
				<!-- конец Блок Импорт упрак2 -->
					...
					<div class="modal fade" id="myModal322" role="dialog"
						 aria-labelledby="myModalLabel" aria-hidden="true">
						<div class="modal-dialog">
							<div class="modal-content">
								<div class="modal-header">
									...
									<p>Отправлен на Гознак, нет полиса</p>
								</div>
								<div class="modal-body">
									<form  method="post" id="formGz">
										<input type="date" name="date_begin" required>
										<input type="date" name="date_end" required>
										<button type="button" class="btn btn-primary" id="taskGzNoPolice">Послать запрос</button>
									</form>
								</div>
								<div class="modal-footer">
									<button type="button" class="btn btn-primary"
					...
					<div class="modal fade" id="myModal323" role="dialog"
						 aria-labelledby="myModalLabel" aria-hidden="true">
						<div class="modal-dialog">
							<div class="modal-content">
								<div class="modal-header">
									<button type="button btn-primary" class="close"
											data-dismiss="modal" aria-hidden="true">&times;</button>
									<h3 class="modal-title">Запрос</h3>
									<p>Сгенерировать енп</p>
								</div>
								<div class="modal-body">
									<h3>Вы уверены?</h3>
									<button class="btn btn-warning" id="taskGenEnpButton"> да </button>
								</div>
								<div class="modal-footer">
									<button type="button" class="btn btn-primary"
											data-dismiss="modal">Закрыть окно</button>
...
<!-- 							<li class="active"><a href="#home-pills" data-toggle="tab">Первая
							...
							<div class="well well-sm">
                                ...
                                <%--<button type = "button btn btn-sm" id="btn_switch_headers">Поменять заголовки</button>--%>
							</div>
						</ul>
